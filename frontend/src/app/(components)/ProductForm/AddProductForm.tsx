...
const AddProductForm: React.FC<AddProductFormProps> = ({ theme, router }) => {
          ...
          if (attr.required) {
            if (attr.type === 'number') {
              acc[attr.name] = Yup.number()
                .min(0, `${attr.name} must be at least 0`)
  ...
  const handleSubmit = async (
    values: Product,
    {
      setSubmitting,
      resetForm,
    }: { setSubmitting: (b: boolean) => void; resetForm: (nextState?: any) => void }
  ) => {
    try {
      ...
      const productData = {
        ...
        discountedPrice: values.discountedPrice
          ? parseFloat(values.discountedPrice.toString())
          : null,
  ...
  const customSelectStyles: StylesConfig<AttributeOption, true> = {
    control: (provided, state) => ({
      ...
      boxShadow: state.isFocused ? `0 0 0 1px ${theme === 'dark' ? '#60a5fa' : '#3b82f6'}` : 'none',
      '&:hover': {
      ...
      backgroundColor: state.isSelected
        ...
        : state.isFocused
          ...
          : theme === 'dark'
            ? '#1f2937'
            : '#ffffff',
      color: state.isSelected ? '#ffffff' : theme === 'dark' ? '#f3f4f6' : '#1f2937',
      '&:hover': {
    ...
    // Dark theme styling for selected chips (multi-value)
    multiValue: (provided) => ({
      ...provided,
      backgroundColor: theme === 'dark' ? '#1f2937' : provided.backgroundColor,
      border: theme === 'dark' ? '1px solid #374151' : provided.border,
      borderRadius: 6,
    }),
    ...
    multiValueRemove: (provided) => ({
      ...provided,
      color: theme === 'dark' ? '#cbd5e1' : provided.color,
      ':hover': {
  ...
  return (
    ...
    >
      {({ values, isSubmitting, setFieldValue }) => (
        <>
          <Form className={`no-spin space-y-6 ${theme} p-4 rounded-lg shadow-lg`}>
              ...
              {selectedCategory?.attributes?.map((attr, index) => (
                <div key={index}>
                  ...
                  {attr.type === 'select' ? (
                    ...
                    <Field
                      type='text'
                      inputMode={attr.type === 'number' ? 'decimal' : undefined}
                      name={`attributes[${attr.name}]`}
