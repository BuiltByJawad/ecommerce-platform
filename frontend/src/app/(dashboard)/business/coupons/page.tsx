...
const VendorCouponsPage: React.FC = () => {
  ...
  const fetchCoupons = async () => {
    try {
      ...
      const res = await get(`/coupons/vendor/mine?${params.toString()}`, {});
      setCoupons(res?.data?.data?.data || []);
    } catch (e: any) {
      const msg = e?.response?.status === 403 ? 'You do not have permission to manage coupons.' : 'Failed to load coupons';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const toggleActive = async (id: string) => {
    try {
      await patch(`/coupons/vendor/${id}/toggle`, {}, {});
  ...
  const saveEdit = async (id: string) => {
    try {
      ...
      const res = await patch(`/coupons/vendor/${id}`, body, {});
  ...
  const onCreate = async () => {
    ...
    try {
      setCreating(true);
      const body: any = { ...form };
      const res = await post('/coupons/vendor', body, {});
      ...
      const msg = e?.response?.status === 403 ? 'You do not have permission to manage coupons.' : 'Create failed';
      toast.error(msg);
    } finally {
      setCreating(false);
    }
  ...
  return (
    <div className={`${theme} p-3`}>
      <h1 className='text-lg font-bold mb-3'>My Coupons</h1>
