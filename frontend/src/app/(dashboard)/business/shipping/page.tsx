...
const VendorShippingPage: React.FC = () => {
  ...
  const fetchVendorRates = async () => {
    try {
      setLoading(true);
      const res = await get('/shipping/vendor/mine', {});
      const arr: RateRow[] = res?.data?.data?.settings?.rates || [];
      setRates(arr);
    } catch (e: any) {
      const msg = e?.response?.status === 403 ? 'You do not have access to shipping settings.' : 'Failed to load shipping settings';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const save = async () => {
    try {
      ...
      await put('/shipping/vendor/mine', { rates: cleaned }, {});
  ...
  return (
    <div className={`${theme} p-3`}>
      <h1 className='text-lg font-bold mb-3'>My Shipping Rates</h1>
