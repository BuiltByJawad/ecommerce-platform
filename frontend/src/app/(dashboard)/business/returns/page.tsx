...
const VendorReturnsPage: React.FC = () => {
  ...
  const user: any = useAppSelector((s) => s.global.currentUser);
  ...
  const hasAccess = user?.role === 'company' && user?.vendorStatus === 'approved' && (permissions.length === 0 || permissions.includes('VIEW_VENDOR_ORDERS'));
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await get(`/returns/vendor/my?${params.toString()}`, {});
      setRows(res?.data?.data?.data || []);
    } catch (e: any) {
      const msg = e?.response?.status === 403 ? 'You do not have access to returns.' : 'Failed to load returns';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => { if (hasAccess) fetchData(); /* eslint-disable-next-line */ }, [hasAccess]);

  const saveStatus = async (id: string, status: string) => {
    try {
      await patch(`/returns/vendor/${id}`, { status }, {});
  ...
  if (!hasAccess) {
    return (
      <div className={`${theme} p-3`}>
        <h1 className='text-lg font-bold mb-2'>Returns</h1>
        <div className='bg-white dark:bg-gray-800 rounded border p-3 text-sm'>
          Your vendor account is not approved or you lack the required permission (VIEW_VENDOR_ORDERS).
        </div>
      </div>
    );
  }
